...
/** The "Parasite" logic. Injects the LSPosed Manager APK into a host process (usually shell). */
...
object ParasiticManagerHooker {
    ...
    private fun getManagerPkgInfo(appInfo: ApplicationInfo?): PackageInfo? {
        if (managerPkgInfo == null && appInfo != null) {
            runCatching {
                    val ctx = ActivityThread.currentActivityThread().systemContext
        ...
        val classLoaderHook =
            object : XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam<*>) {
                    ...
                    if (mAppInfo == pkgInfo.applicationInfo) {
                        ...
                        this.unhook() // Only need to inject once
                    }
                }
            }
        XposedHelpers.findAndHookMethod(LoadedApk::class.java, "getClassLoader", classLoaderHook)
    ...
    fun start(): Boolean {
        ...
        return try {
            serviceClient.requestInjectedManagerBinder(binderList).use { pfd ->
                if (binderList.isNotEmpty() && binderList[0] != null && pfd != null) {
